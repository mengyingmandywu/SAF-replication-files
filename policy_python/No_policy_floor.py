...
# Add parent directory to sys.path to import generate_outputs
...
parser = argparse.ArgumentParser(description='Run biofuel policy optimization model (No_policy_floor) with configurable input/output folders.')
...
input_dir = os.path.join(parent_dir, args.input_folder)
results_dir = os.path.join(parent_dir, args.output_folder)
...
print(f"Intermediate folder: {intermediate_dir}")

# Create the solution DataFrame
solution = pd.DataFrame(
    np.zeros((76, 6)),  # 70 rows and 6 columns filled with zeros
...
Total['Name'] = [
    ...
    'Taxpayer_D', 'Taxpayer_G', 'Taxpayer_J'
]
...
for k in range(K_RANGE):    
    # Generate input_biofuel_V1.xlsx if it doesn't exist
    input_biofuel_path = os.path.join(intermediate_dir, 'input_biofuel_V1.xlsx')
    if not os.path.exists(input_biofuel_path):
        print(f"Generating {input_biofuel_path}...")
        generate_outputs.main(input_dir, intermediate_dir)
    
    # Read input data from intermediate folder
    input_biofuel = pd.read_excel(input_biofuel_path, sheet_name='Sheet1', index_col=0)
    
    # Drop old CI_std_LCFS column and rename CI_std_LCFS_new to CI_std_LCFS
    ...
    feedstock_path = os.path.join(input_dir, 'feedstock_supply.xlsx')
    input_feedstock = pd.read_excel(feedstock_path, index_col=0)
    
    demand_path = os.path.join(input_dir, 'fuel_demand.xlsx')
    ...
    compliance_path = os.path.join(input_dir, 'Policy_constraints.xlsx')
    ...
    state_tax_path = os.path.join(input_dir, 'state_tax.xlsx')
    ...
    for break_idx, break_val in enumerate(breaks):
        ...
        # Create a new model
        m = gp.Model("Current_policy_noLCFS")   
        ...
        conventionalfuel_quantity.loc['B0':'E0', 'CI_LCFS'] = input_compliance.loc[input_compliance['mandate'] == 'LCFS', 'CI_LCFS'].values
        conventionalfuel_quantity.loc['B0', 'CI_std_LCFS'] = input_biofuel.loc[(input_biofuel['F'] == 'D') & (input_biofuel['j'] == 'CA'), 'CI_std_LCFS'].unique()[0]
        conventionalfuel_quantity.loc['E0', 'CI_std_LCFS'] = input_biofuel.loc[(input_biofuel['F'] == 'G') & (input_biofuel['j'] == 'CA'), 'CI_std_LCFS'].unique()[0]
        conventionalfuel_quantity.loc['J0', 'CI_std_LCFS'] = input_biofuel.loc[(input_biofuel['F'] == 'J') & (input_biofuel['j'] == 'CA'), 'CI_std_LCFS'].unique()[0]
        conventionalfuel_quantity.loc['B0':'E0', 'ED_LCFS'] = input_compliance.loc[input_compliance['mandate'] == 'LCFS', 'ED_LCFS'].values
        ...
        for i in range(24):
            Constraints.append(
                ...
                # ((47.39 - output['CI_Tax'][i]) / 47.39) * output['Taxheavyside'][i] 
                # #+ (output['f'][i] == 'SAF') * SAFcredit_prices['P'].item() 
                ...
                # ((output['f'][i] == 'SAF') & (output['j'][i] == 'NC')) * output['Taxheavyside'][i]* val + 
                mu['value'][i] -
                nu.loc[(nu['f'] == output['f'][i]), 'value'].item()  
                # + ((output['f'][i] == 'SAF') & (output['j'][i] == 'NC')) * output['Taxheavyside'][i]* diracdeltalb_value - 
                #     ((output['f'][i] == 'SAF') & (output['j'][i] == 'NC')) * output['Taxheavyside'][i]* diracdeltaub_value  
        ...
        for fuel in fuel_types:
            ...
            Constraints.append(nu_value[nu_index.index(f'{fuel}')] <= (1 - nuzeta_value[nu_index.index(f'{fuel}')]) * M5)
            ...
            Constraints.append(-(biofuel_quantity.loc[(biofuel_quantity['f'] == fuel), 'Q'].sum() -
                                ...
                                nuzeta_value[nu_index.index(f'{fuel}')] * M5)
        ...
        for fuel in fuel_types:
            Constraints.append(nu_value[nu_index.index(f'{fuel}')] >= 0)
            Constraints.append(nu_value[nu_index.index(f'{fuel}')] <= (1 - nuzeta_value[nu_index.index(f'{fuel}')]) * M6)
            Constraints.append(-(biofuel_quantity.loc[(biofuel_quantity['f'] == fuel), 'Q'].sum() -
                                        biofuel_quantity.loc[(biofuel_quantity['f'] == fuel), 'BW_constraints'].unique()[0] *
            ...
            Constraints.append(-(biofuel_quantity.loc[(biofuel_quantity['f'] == fuel), 'Q'].sum() -
                                ...
                                nuzeta_value[nu_index.index(f'{fuel}')] * M6)
        ...
        for fuel in fuel_types:
            Constraints.append(nu_value[nu_index.index(f'{fuel}')] >= 0)
            Constraints.append(nu_value[nu_index.index(f'{fuel}')] <= (1 - nuzeta_value[nu_index.index(f'{fuel}')]) * M7)
            Constraints.append(-(biofuel_quantity.loc[(biofuel_quantity['f'] == fuel), 'Q'].sum() -
                                        biofuel_quantity.loc[(biofuel_quantity['f'] == fuel), 'BW_constraints'].unique()[0] *
            ...
            Constraints.append(-(biofuel_quantity.loc[(biofuel_quantity['f'] == fuel), 'Q'].sum() -
                                ...
                                nuzeta_value[nu_index.index(f'{fuel}')] * M7)        
        ...
        # Constraints.append((biofuel_quantity.loc[(biofuel_quantity['f'] == 'SAF') & (biofuel_quantity['j'] == 'NC'), 'Q'].sum() - epsilon1 -
        #         break_val_pre) >= 0)         
        # Constraints.append(biofuel_quantity.loc[(biofuel_quantity['f'] == 'SAF') & (biofuel_quantity['j'] == 'NC'), 'Q'].sum() -
        ...
        # total_diesel_qty = (
        #     # Conventional diesel quantities
        #     conventionalfuel_quantity.loc[(conventionalfuel_quantity['F'] == 'D'), 'Q'].item() +
        #     # Biofuel diesel quantities (B100 and RD)
        #     sum(biofuel_quantity.loc[biofuel_quantity['f'].isin(['B100', 'RD']), 'Q'])
        # )
        ...
        #     conventionalfuel_quantity.loc[(conventionalfuel_quantity['F'] == 'G'), 'Q'].item()  +
        ...
        # Conventional fuel prices
        Fconventionalfuel_pricesD = (conventionalfuel_prices.loc[(conventionalfuel_prices['F'] == 'D'), 'P'].item()  - 
                                        input_demand.loc[input_demand['F'] == 'D', 'PC_F0'].unique()[0])
        Fconventionalfuel_pricesG = (conventionalfuel_prices.loc[(conventionalfuel_prices['F'] == 'G'), 'P'].item()  - 
                                        input_demand.loc[input_demand['F'] == 'G', 'PC_F0'].unique()[0])
        Fconventionalfuel_pricesJ = (conventionalfuel_prices.loc[(conventionalfuel_prices['F'] == 'J'), 'P'].item() - 
        ...
        M9 = 1
        m.addConstr(blending_ratio >= 0.1 + Sigma - M9 * (1 - indicator_var), name="indicator_var_constr1")
        m.addConstr(blending_ratio <= 0.1 + M9 * indicator_var, name="indicator_var_constr2")
        ...
        for i in range(24):
            Constraints.append(
                Price.iloc[i]['P_all'] - (biofuel_prices.loc[(biofuel_prices['f'] == output.iloc[i]['f']), 'P'].item()) == 0
            )

        # Add constraints for Price DataFrame
        Constraints.append(
            Price.loc[(Price['f'] == 'B0'), 'P_all'].values[0] -
            input_demand.loc[input_demand['F'] == 'D', 'PC_F0'].values[0] == 0
        )
        Constraints.append(
            Price.loc[(Price['f'] == 'E0'), 'P_all'].values[0] -
            input_demand.loc[input_demand['F'] == 'G', 'PC_F0'].values[0] == 0
        )
        Constraints.append(
            Price.loc[(Price['f'] == 'J0'), 'P_all'].values[0] -
        ...
        for i in range(24):
            Constraints.append(
                Price.iloc[i]['P_RIN_LCFS_detach'] - biofuel_prices.loc[(biofuel_prices['f'] == output.iloc[i]['f']), 'P'].item() == 0
            )

        Constraints.append(
            Price.loc[(Price['f'] == 'B0'), 'P_RIN_LCFS_detach'].item() -
            input_demand.loc[input_demand['F'] == 'D', 'PC_F0'].unique()[0] == 0
        )
        Constraints.append(
            Price.loc[(Price['f'] == 'E0'), 'P_RIN_LCFS_detach'].item() -
            input_demand.loc[input_demand['F'] == 'G', 'PC_F0'].unique()[0] == 0
        )
        Constraints.append(
        ...
        for i in range(24):
            Constraints.append(
                Price.iloc[i]['P_RIN_detach'] - (
                    biofuel_prices.loc[(biofuel_prices['f'] == output.iloc[i]['f']), 'P'].item()
                ) == 0
            )
        ...
        for feedstock in ['corn', 'sugarcane']:
        #for feedstock in feedstocks: 
        # because of cap we only need to clear corn and sugarcane
            demand = (
        ...
        # Add constraints that feedstock prices have to be larger than 0.5
        for feedstock in feedstocks:
            Constraints.append(feedstock_prices.loc[feedstock, 'P'] >= 0.5)    
        ...
        # Constraints.append((biofuel_quantity.loc[biofuel_quantity['f'].isin(['B100', 'RD', 'SAF']), 'Q'].sum() -
        #     RVO_percent.loc['BBD', 'P'] / 1.6 * denominator) <= PD4zeta * M11)
        ...
        # Constraints.append((sum((biofuel_quantity.loc[biofuel_quantity['f'] == 'B100', 'kappa_RFS'] * biofuel_quantity.loc[biofuel_quantity['f'] == 'B100', 'Q'])) + sum(biofuel_quantity.loc[biofuel_quantity['f'] == 'RD', 'kappa_RFS'] * biofuel_quantity.loc[biofuel_quantity['f'] == 'RD', 'Q']) + sum(biofuel_quantity.loc[biofuel_quantity['f'] == 'SAF', 'kappa_RFS'] * biofuel_quantity.loc[biofuel_quantity['f'] == 'SAF', 'Q']) + sum(biofuel_quantity.loc[biofuel_quantity['f'] == 'gasE100', 'kappa_RFS'] * biofuel_quantity.loc[biofuel_quantity['f'] == 'gasE100', 'Q']) - RVO_percent.loc['RF', 'P'] * denominator) >= 0)
        # Constraints.append((sum((biofuel_quantity.loc[biofuel_quantity['f'] == 'B100', 'kappa_RFS'] * biofuel_quantity.loc[biofuel_quantity['f'] == 'B100', 'Q'])) + sum(biofuel_quantity.loc[biofuel_quantity['f'] == 'RD', 'kappa_RFS'] * biofuel_quantity.loc[biofuel_quantity['f'] == 'RD', 'Q']) + sum(biofuel_quantity.loc[biofuel_quantity['f'] == 'SAF', 'kappa_RFS'] * biofuel_quantity.loc[biofuel_quantity['f'] == 'SAF', 'Q']) +sum(biofuel_quantity.loc[biofuel_quantity['f'] == 'gasE100', 'kappa_RFS'] * biofuel_quantity.loc[biofuel_quantity['f'] == 'gasE100', 'Q']) - RVO_percent.loc['RF', 'P'] * denominator) <= PD6zeta * M12)
...
#                                                 np.log(2)) - 1))        
    ...
    if best_df is not None:
        ...
        if k == 0:
            append_to_excel(f'{output_dir1}Mean.xlsx', Intermediate_var, 'No_policy_floor')

        if k == 1:
            append_to_excel(f'{output_dir1}10th.xlsx', Intermediate_var, 'No_policy_floor')

        if k == 2:
            append_to_excel(f'{output_dir1}33th.xlsx', Intermediate_var, 'No_policy_floor')

        if k == 3:
            append_to_excel(f'{output_dir1}67th.xlsx', Intermediate_var, 'No_policy_floor') 

        if k == 4:
            append_to_excel(f'{output_dir1}90th.xlsx', Intermediate_var, 'No_policy_floor')
        ...
        solution.iloc[:, k+1] = (
            ...
            + [0]
            + [0,0]
            + [best_model.getVarByName(feedstock_prices_P[i].VarName).X for i in range(2, 4)]
            + [0, 0] + [best_model.getVarByName(feedstock_quantity_Q[i].VarName).X for i in range(2, 4)]
            + [0]
        ...
        Fitted_quantity.iloc[21:25, k+1] = [0, 0] + [best_model.getVarByName(feedstock_quantity_Q[i].VarName).X for i in range(2, 4)]
        ...
        Fuel_price.iloc[22:26, k+1] = [
            ...
            sum(Intermediate_var.loc[Intermediate_var['f'] == 'SAF', 'P_all'] *Intermediate_var.loc[Intermediate_var['f'] == 'SAF', 'Q_actual']) /sum(Intermediate_var.loc[Intermediate_var['f'] == 'SAF', 'Q_equiv'])  if sum(Intermediate_var.loc[Intermediate_var['f'] == 'SAF', 'Q_equiv']) != 0 else np.nan
        ]
...
append_to_excel(f'{output_dir}Solution.xlsx', solution, 'No_policy_floor')
append_to_excel(f'{output_dir}Fitted_quantity.xlsx', Fitted_quantity, 'No_policy_floor')
append_to_excel(f'{output_dir}Fuel_price.xlsx', Fuel_price, 'No_policy_floor')
append_to_excel(f'{output_dir}Fuel_CI.xlsx', Fuel_CI, 'No_policy_floor')
append_to_excel(f'{output_dir}Total.xlsx', Total, 'No_policy_floor')
append_to_excel(f'{output_dir}Feedstock_source.xlsx', Feedstock_source, 'No_policy_floor')
