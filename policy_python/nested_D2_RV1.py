...
parser = argparse.ArgumentParser(description='Run biofuel policy optimization model (nested_D2_RV1) with configurable input/output folders.')
...
for k in range(K_RANGE):    
    ...
    print(input_compliance.loc[input_compliance['mandate'] == 'LCFS', 'CI_LCFS'].values)
    ...
    epsilon = 1e-4
    Sigma = 1e-5 #Should be ≳ feasibility tol
    iteration = 0
    ...
    for break_idx, break_val in enumerate(breaks):
        ...
        Constraints.append(
            RVO_percent.loc['D2', 'P'] == 
            3*1.6 / conventionalfuel_quantity.loc[(conventionalfuel_quantity['F'] == 'G') | (conventionalfuel_quantity['F'] == 'D'), 'Q'].sum()
        )
        ...
        Constraints.append(
            RVO_percent.loc['BBD', 'P'] == 
            biofuel_quantity.loc[(biofuel_quantity['D4'] == 1) | (biofuel_quantity['D2'] == 1), 'Q'].sum() * 1.6 / 
            conventionalfuel_quantity.loc[(conventionalfuel_quantity['F'] == 'G') | (conventionalfuel_quantity['F'] == 'D'), 'Q'].sum()
        )
        ...
        # # Create variable for max(RVO_percent.loc['BBD', 'P'] - RVO_percent.loc['D2', 'P'], 0) using big-M
        # max_BBD_D2_diff = m.addVar(lb=0, name="max_BBD_D2_diff")
        # binary_positive = m.addVar(vtype=GRB.BINARY, name="binary_positive")
        
        # bigM = 10
        # # If RVO_percent.loc['BBD', 'P'] - RVO_percent.loc['D2', 'P'] > 0, then max_BBD_D2_diff = RVO_percent.loc['BBD', 'P'] - RVO_percent.loc['D2', 'P']
        # # If RVO_percent.loc['BBD', 'P'] - RVO_percent.loc['D2', 'P'] <= 0, then max_BBD_D2_diff = 0
        # m.addConstr(max_BBD_D2_diff >= RVO_percent.loc['BBD', 'P'] - RVO_percent.loc['D2', 'P'])
        # m.addConstr(max_BBD_D2_diff <= (RVO_percent.loc['BBD', 'P'] - RVO_percent.loc['D2', 'P']) + bigM * (1 - binary_positive))
        # m.addConstr(max_BBD_D2_diff <= bigM * binary_positive)
        ...
        Fconventionalfuel_pricesD_NC = (conventionalfuel_prices.loc[(conventionalfuel_prices['F'] == 'D') & (conventionalfuel_prices['j'] == 'NC'), 'P'].item() -  RVO_percent.loc['D2', 'P'] * RIN_prices.loc['D2', 'P'] - 
            (RVO_percent.loc['BBD', 'P'] - RVO_percent.loc['D2', 'P']) * RIN_prices.loc['D4', 'P'] - 
        ...
        Fconventionalfuel_pricesJ_NC = (conventionalfuel_prices.loc[(conventionalfuel_prices['F'] == 'J') & (conventionalfuel_prices['j'] == 'NC'), 'P'].item() - 
        ...
        Fconventionalfuel_pricesG_CA = (conventionalfuel_prices.loc[(conventionalfuel_prices['F'] == 'G') & (conventionalfuel_prices['j'] == 'CA'), 'P'].item() -  RVO_percent.loc['D2', 'P'] * RIN_prices.loc['D2', 'P'] - 
            ...
            (RVO_percent.loc['RF', 'P'] - RVO_percent.loc['BBD', 'P']) * RIN_prices.loc['D6', 'P'] - 
                                        ...
                                        input_demand.loc[input_demand['F'] == 'G', 'PC_F0'].unique()[0])
        Fconventionalfuel_pricesJ_CA = (conventionalfuel_prices.loc[(conventionalfuel_prices['F'] == 'J') & (conventionalfuel_prices['j'] == 'CA'), 'P'].item() - 
                                        input_demand.loc[input_demand['F'] == 'J', 'PC_F0'].unique()[0])
        ...
        Constraints.append(
            ...
            RVO_percent.loc['D2', 'P'] * RIN_prices.loc['D2', 'P'] - 
            (RVO_percent.loc['BBD', 'P'] - RVO_percent.loc['D2', 'P']) * RIN_prices.loc['D4', 'P'] - 
            (RVO_percent.loc['RF', 'P'] - RVO_percent.loc['BBD', 'P']) * RIN_prices.loc['D6', 'P'] == 0
        )
        Constraints.append(
        ...
        Constraints.append(
            Price.loc[(Price['f'] == 'J0') & (Price['j'] == 'NC'), 'P_RIN_LCFS_detach'].item() -
        ...
        Constraints.append(
            Price.loc[(Price['f'] == 'J0') & (Price['j'] == 'CA'), 'P_RIN_LCFS_detach'].item() -
            input_demand.loc[input_demand['F'] == 'J', 'PC_F0'].unique()[0] == 0
        )
        ...
        Constraints.append(
            Price.loc[(Price['f'] == 'J0') & (Price['j'] == 'CA'), 'P_RIN_detach'].item() -
            input_demand.loc[input_demand['F'] == 'J', 'PC_F0'].unique()[0] == 0
        )
        ...
        M11_0 = 10
        Constraints.append(RIN_prices.loc['D2', 'P'] - RIN_prices.loc['D4', 'P'] >= 0)
        Constraints.append(RIN_prices.loc['D2', 'P'] - RIN_prices.loc['D4', 'P'] <= (1 - PD2zeta) * M11_0)
        ...
        # Constraints.append(sum(biofuel_quantity.loc[biofuel_quantity['f'] == 'SAF', 'Q']) - input_compliance.loc[input_compliance['mandate'] == 'SAF', 'Q'].item() <= PD2zeta * M11_0)
        ...
        # Constraints.append(sum(biofuel_quantity.loc[biofuel_quantity['f'] == 'SAF', 'Q']) == input_compliance.loc[input_compliance['mandate'] == 'SAF', 'Q'].item())
        ...
        # Constraints.append((biofuel_quantity.loc[biofuel_quantity['f'].isin(['B100', 'RD', 'SAF']), 'Q'].sum() -
        #     RVO_percent.loc['BBD', 'P'] / 1.6 * conventionalfuel_quantity.loc[conventionalfuel_quantity['F'].isin(['G', 'D']), 'Q'].sum()) <= PD4zeta * M11_1)

        # #SAF ATJ without CCS only get D6 RINs
        # Constraints.append((biofuel_quantity.loc[(biofuel_quantity['D4'] == 1) | (biofuel_quantity['D2'] == 1), 'Q'].sum() -
        #     RVO_percent.loc['BBD', 'P'] / 1.6 * conventionalfuel_quantity.loc[conventionalfuel_quantity['F'].isin(['G', 'D']), 'Q'].sum()) >= 0)
        # Constraints.append((biofuel_quantity.loc[(biofuel_quantity['D4'] == 1) | (biofuel_quantity['D2'] == 1), 'Q'].sum() -
        #     RVO_percent.loc['BBD', 'P'] / 1.6 * conventionalfuel_quantity.loc[conventionalfuel_quantity['F'].isin(['G', 'D']), 'Q'].sum()) <= PD4zeta * M11_1)
        ...
        # Constraints.append((biofuel_quantity.loc[biofuel_quantity['f'].isin(['B100', 'RD', 'SAF']), 'Q'].sum() -
        #     RVO_percent.loc['BBD', 'P'] / 1.6 * denominator) >= 0)
        # Constraints.append((biofuel_quantity.loc[biofuel_quantity['f'].isin(['B100', 'RD', 'SAF']), 'Q'].sum() -
        #     RVO_percent.loc['BBD', 'P'] / 1.6 *denominator) <= PD4zeta * M11_1)
    ...
    if best_df is not None:
        ...
        for i in range(24):
            ...
            Intermediate_var.at[i, 'Production_cost'] = (
                output['PC'][i]
                + output['Conversion'][i]
                * best_model.getVarByName(feedstock_prices.loc[feedstock_prices['s'] == output['s'][i], 'P'].item().VarName).X
                *(1+duty_e100sugarcane*(output['s'][i] == 'sugarcane'))
            )
            ...
            Intermediate_var.at[i, 'foc'] = (
                ...
                * best_model.getVarByName(feedstock_prices.loc[feedstock_prices['s'] == output['s'][i], 'P'].item().VarName).X*(1+duty_e100sugarcane*(output['s'][i] == 'sugarcane')))
                ...
                + 1 * ((output['f'][i] == 'B100') | (output['f'][i] == 'RD') | (output['f'][i] == 'gasE100'))
                * ((47.39 - output['CI_Tax'][i]) / 47.39) * (output['s'][i] != 'sugarcane')
                * output['Taxheavyside'][i]
                + max([
                    1 * (output['f'][i] == 'SAF')
                    * ((47.39 - output['CI_Tax'][i]) / 47.39)
        ...
        if k == 0:
            append_to_excel(os.path.join(output_dir1, 'Mean.xlsx'), Intermediate_var, 'Nested_D2_RV1')

        if k == 1:
            append_to_excel(os.path.join(output_dir1, '10th.xlsx'), Intermediate_var, 'Nested_D2_RV1')

        if k == 2:
            append_to_excel(os.path.join(output_dir1, '33th.xlsx'), Intermediate_var, 'Nested_D2_RV1')

        if k == 3:
            append_to_excel(os.path.join(output_dir1, '67th.xlsx'), Intermediate_var, 'Nested_D2_RV1') 

        if k == 4:
            append_to_excel(os.path.join(output_dir1, '90th.xlsx'), Intermediate_var, 'Nested_D2_RV1')
        ...
        Fuel_price.iloc[24, k+1] = [
            ...
            #best_model.getVarByName(max_BBD_D2_diff.VarName).X * best_model.getVarByName(RIN_prices_P[1].VarName).X +
            (best_model.getVarByName(RVO_percent_P[2].VarName).X - best_model.getVarByName(RVO_percent_P[1].VarName).X) * best_model.getVarByName(RIN_prices_P[2].VarName).X
        ]
...
# Define the output directory (use the results_dir that was set earlier)
...
append_to_excel(os.path.join(output_dir, 'Solution.xlsx'), solution, 'Nested_D2_RV1')
append_to_excel(os.path.join(output_dir, 'Fitted_quantity.xlsx'), Fitted_quantity, 'Nested_D2_RV1')
append_to_excel(os.path.join(output_dir, 'Fuel_price.xlsx'), Fuel_price, 'Nested_D2_RV1')
append_to_excel(os.path.join(output_dir, 'Fuel_CI.xlsx'), Fuel_CI, 'Nested_D2_RV1')
append_to_excel(os.path.join(output_dir, 'Total.xlsx'), Total, 'Nested_D2_RV1')
append_to_excel(os.path.join(output_dir, 'Feedstock_source.xlsx'), Feedstock_source, 'Nested_D2_RV1')
