...
parser = argparse.ArgumentParser(description='Run biofuel policy optimization model with configurable input/output folders.')
...
for k in range(K_RANGE):    
    ...
    for break_idx, break_val in enumerate(breaks):
        ...
        # Constraints.append(diracdeltalb_value <= diraczetalb_value * M8_lb)
        ...
        #         break_val_pre - epsilon1 <= (1-diraczetalb_value) * M8_lb) 
...
# print((nlfunc.exp(((sum(biofuel_quantity.loc[(biofuel_quantity['f'] == 'gasE100') & (biofuel_quantity['j'] == 'NC'), 'Q']) / 
#                                                 demand_quantity.loc[(demand_quantity['F'] == 'G') & (demand_quantity['j'] == 'NC'), 'Q_actual'].item() - 0.1) / 0.02) * 
#                                                 np.log(2)) - 1))        
    ...
    if best_df is not None:
        ...
        if k == 0:
            append_to_excel(os.path.join(output_dir1, 'Mean.xlsx'), Intermediate_var, 'Current_policy')

        if k == 1:
            append_to_excel(os.path.join(output_dir1, '10th.xlsx'), Intermediate_var, 'Current_policy')

        if k == 2:
            append_to_excel(os.path.join(output_dir1, '33th.xlsx'), Intermediate_var, 'Current_policy')

        if k == 3:
            append_to_excel(os.path.join(output_dir1, '67th.xlsx'), Intermediate_var, 'Current_policy') 

        if k == 4:
            append_to_excel(os.path.join(output_dir1, '90th.xlsx'), Intermediate_var, 'Current_policy')
        ...
        intermediate_input_path = os.path.join(intermediate_dir, 'Input_intermediate_V1.xlsx')
        # intermediate_input_path = os.path.join(script_dir, 'Input_intermediate.xlsx')
        
        # # Verify the file exists with fallback options
        # if not os.path.exists(intermediate_input_path):
        #     # Try current working directory as backup
        #     intermediate_input_backup = 'Input_intermediate.xlsx'
        #     if os.path.exists(intermediate_input_backup):
        #         intermediate_input_path = intermediate_input_backup
        #         print(f"Using Input_intermediate.xlsx from current directory: {intermediate_input_path}")
        #     else:
        #         raise FileNotFoundError(f"Input_intermediate.xlsx not found. Tried:\n1. {intermediate_input_path}\n2. {intermediate_input_backup}")
        # else:
        #     print(f"Using Input_intermediate.xlsx: {intermediate_input_path}")
        ...
        Fuel_price.iloc[12:16, k+1] = [
            sum(Intermediate_var.loc[Intermediate_var['f'] == 'B100', 'P_all'] *Intermediate_var.loc[Intermediate_var['f'] == 'B100', 'Q_actual']) /sum(Intermediate_var.loc[Intermediate_var['f'] == 'B100', 'Q_actual']),
        ...
        # Total.iloc[1:5, k+1] = [
        #     sum(Intermediate_var.loc[Intermediate_var['F'] == 'D', 'Q_actual'] *
        #         Intermediate_var.loc[Intermediate_var['F'] == 'D', 'CI_45Z'] *
        #         Intermediate_var.loc[Intermediate_var['F'] == 'D', 'ED_LCFS']) / 10**6,
        #     sum(Intermediate_var.loc[Intermediate_var['F'] == 'G', 'Q_actual'] *
        #         Intermediate_var.loc[Intermediate_var['F'] == 'G', 'CI_45Z'] *
        #         Intermediate_var.loc[Intermediate_var['F'] == 'G', 'ED_LCFS']) / 10**6,
        #     sum(Intermediate_var.loc[Intermediate_var['F'] == 'J', 'Q_actual'] *
        #         Intermediate_var.loc[Intermediate_var['F'] == 'J', 'CI_45Z'] *
        #         Intermediate_var.loc[Intermediate_var['F'] == 'J', 'ED_LCFS']) / 10**6,
        #     sum(Intermediate_var['Q_actual'] * Intermediate_var['CI_45Z'] * Intermediate_var['ED_LCFS']) / 10**6
        # ]
...
append_to_excel(f'{output_dir}Solution.xlsx', solution, 'Current_policy')
append_to_excel(f'{output_dir}Fitted_quantity.xlsx', Fitted_quantity, 'Current_policy')
append_to_excel(f'{output_dir}Fuel_price.xlsx', Fuel_price, 'Current_policy')
append_to_excel(f'{output_dir}Fuel_CI.xlsx', Fuel_CI, 'Current_policy')
append_to_excel(f'{output_dir}Total.xlsx', Total, 'Current_policy')
append_to_excel(f'{output_dir}Feedstock_source.xlsx', Feedstock_source, 'Current_policy')
