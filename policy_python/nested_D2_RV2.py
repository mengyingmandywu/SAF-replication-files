...
parser = argparse.ArgumentParser(description='Run biofuel policy optimization model (nested_D2_RV2) with configurable input/output folders.')
...
for k in range(K_RANGE):    
    ...
    for break_idx, break_val in enumerate(breaks):
        ...
        #     RVO_percent.loc['RF', 'P'] == 
        #     (input_compliance.loc[input_compliance['mandate'] == 'RF', 'Q'].item() * 
        #     conventionalfuel_quantity.loc[(conventionalfuel_quantity['F'] == 'G') | (conventionalfuel_quantity['F'] == 'D'), 'Q'].sum()+3*1.6) / 
        #     conventionalfuel_quantity.loc[(conventionalfuel_quantity['F'] == 'G') | (conventionalfuel_quantity['F'] == 'D'), 'Q'].sum()
        # )
        ...
        Constraints.append((biofuel_quantity.loc[(biofuel_quantity['D4'] == 1) | (biofuel_quantity['D2'] == 1), 'Q'].sum() -
            (3+input_compliance.loc[input_compliance['mandate'] == 'BBD', 'Q'].item() / 1.6 * conventionalfuel_quantity.loc[conventionalfuel_quantity['F'].isin(['G', 'D']), 'Q'].sum())) >= 0)
        Constraints.append((biofuel_quantity.loc[(biofuel_quantity['D4'] == 1) | (biofuel_quantity['D2'] == 1), 'Q'].sum() -
            (3+input_compliance.loc[input_compliance['mandate'] == 'BBD', 'Q'].item() / 1.6 * conventionalfuel_quantity.loc[conventionalfuel_quantity['F'].isin(['G', 'D']), 'Q'].sum())) <= PD4zeta * M11_1)
    ...
    if best_df is not None:
        ...
        if k == 0:
            append_to_excel(os.path.join(output_dir1, 'Mean.xlsx'), Intermediate_var, 'Nested_D2_RV2')

        if k == 1:
            append_to_excel(os.path.join(output_dir1, '10th.xlsx'), Intermediate_var, 'Nested_D2_RV2')

        if k == 2:
            append_to_excel(os.path.join(output_dir1, '33th.xlsx'), Intermediate_var, 'Nested_D2_RV2')

        if k == 3:
            append_to_excel(os.path.join(output_dir1, '67th.xlsx'), Intermediate_var, 'Nested_D2_RV2') 

        if k == 4:
            append_to_excel(os.path.join(output_dir1, '90th.xlsx'), Intermediate_var, 'Nested_D2_RV2')
...
append_to_excel(os.path.join(output_dir, 'Solution.xlsx'), solution, 'Nested_D2_RV2')
append_to_excel(os.path.join(output_dir, 'Fitted_quantity.xlsx'), Fitted_quantity, 'Nested_D2_RV2')
append_to_excel(os.path.join(output_dir, 'Fuel_price.xlsx'), Fuel_price, 'Nested_D2_RV2')
append_to_excel(os.path.join(output_dir, 'Fuel_CI.xlsx'), Fuel_CI, 'Nested_D2_RV2')
append_to_excel(os.path.join(output_dir, 'Total.xlsx'), Total, 'Nested_D2_RV2')
append_to_excel(os.path.join(output_dir, 'Feedstock_source.xlsx'), Feedstock_source, 'Nested_D2_RV2')
