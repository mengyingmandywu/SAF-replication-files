...
# 45Z CI difference make it more attractive to consume biofuels in CA instead of NC
# no longer uses the old methods
...
parser = argparse.ArgumentParser(description='Run biofuel policy optimization model (noLCFS) with configurable input/output folders.')
...
for k in range(K_RANGE):    
    ...
    for break_idx, break_val in enumerate(breaks):
        ...
        m.addConstr(LCFS_prices_P == 0, name='no_LCFS_price')
...
#                                                 np.log(2)) - 1))        
    ...
    if best_df is not None:
        ...
        if k == 0:
            append_to_excel(os.path.join(output_dir1, 'Mean.xlsx'), Intermediate_var, 'Current_policy_no_LCFS')

        if k == 1:
            append_to_excel(os.path.join(output_dir1, '10th.xlsx'), Intermediate_var, 'Current_policy_no_LCFS')

        if k == 2:
            append_to_excel(os.path.join(output_dir1, '33th.xlsx'), Intermediate_var, 'Current_policy_no_LCFS')

        if k == 3:
            append_to_excel(os.path.join(output_dir1, '67th.xlsx'), Intermediate_var, 'Current_policy_no_LCFS') 

        if k == 4:
            append_to_excel(os.path.join(output_dir1, '90th.xlsx'), Intermediate_var, 'Current_policy_no_LCFS')
        ...
        Fuel_price.iloc[30:34, k+1] = [
            ...
            sum(Intermediate_var.loc[Intermediate_var['f'] == 'RD', 'P_all'] * Intermediate_var.loc[Intermediate_var['f'] == 'RD', 'Q_actual']) /sum(Intermediate_var.loc[Intermediate_var['f'] == 'RD', 'Q_equiv']) if sum(Intermediate_var.loc[Intermediate_var['f'] == 'RD', 'Q_equiv']) != 0 else np.nan,   
...
append_to_excel(os.path.join(results_dir, 'Solution.xlsx'), solution, 'Current_policy_no_LCFS')
append_to_excel(os.path.join(results_dir, 'Fitted_quantity.xlsx'), Fitted_quantity, 'Current_policy_no_LCFS')
append_to_excel(os.path.join(results_dir, 'Fuel_price.xlsx'), Fuel_price, 'Current_policy_no_LCFS')
append_to_excel(os.path.join(results_dir, 'Fuel_CI.xlsx'), Fuel_CI, 'Current_policy_no_LCFS')
append_to_excel(os.path.join(results_dir, 'Total.xlsx'), Total, 'Current_policy_no_LCFS')
append_to_excel(os.path.join(results_dir, 'Feedstock_source.xlsx'), Feedstock_source, 'Current_policy_no_LCFS')
