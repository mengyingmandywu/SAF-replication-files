...
parser = argparse.ArgumentParser(description='Run biofuel policy optimization model (Pure_quantity) with configurable input/output folders.')
...
Fuel_price['Name'] = [
    ...
    'Implicit tax', 'RFS_RVO_road','RFS_RVO_jet', 'LCFS_CA_diesel', 'LCFS_CA_gasoline','Pure_quantity',
...
for k in range(K_RANGE):    
    ...
    v2_path = os.path.join(intermediate_dir, 'input_biofuel_V2.xlsx')
    if not os.path.exists(v2_path):
        ...
        if _generate_outputs_main is not None:
            try:
                ...
                raise RuntimeError(f"Failed to generate input_biofuel_V2.xlsx via generate_outputs.py: {e}")
        else:
            raise FileNotFoundError(
                "input_biofuel_V2.xlsx not found and generate_outputs.py unavailable to build it."
            )
    if not os.path.exists(v2_path):
        raise FileNotFoundError(f"Expected generated file is missing: {v2_path}")

    input_biofuel = pd.read_excel(v2_path)
    ...
    for break_idx, break_val in enumerate(breaks):
        ...
        for i in range(24):
            Constraints.append(
                ...
                + (output['f'][i] == 'SAF') * SAFcredit_prices['P'].item() * output['SAFcreditheaviside'][i]
        ...
        for i in range(24):
            Constraints.append(
                Price.iloc[i]['Tax'] - ((output['f'][i] == 'SAF') * SAFcredit_prices['P'].item() * output.iloc[i]['SAFcreditheaviside'] +
        ...
        for i in range(24):
            Constraints.append(
                Price.iloc[i]['P_all'] - ((output['f'][i] == 'SAF') * SAFcredit_prices['P'].item() * output.iloc[i]['SAFcreditheaviside'] +
        ...
        # #here the denominator include not only transportation fuels, but also other sectors
        ...
        if m.status == GRB.OPTIMAL:
            ...
            for i in range(24):
                ...
                v14 = biofuel_quantity_Q[i].X * (output['CI_std_LCFS'][i] - output['CI_LCFS'][i]) * output['ED_LCFS'][i] * (output['j'][i] == 'CA') / 1000000
                # ... calculate v1, v2, ..., v11 ...
                results.append({
                    'i': i,
                    'Name': names[i],
    ...
    if best_df is not None:
        ...
        print(f"Break {break_idx}: break = {break_val}, val = {val},val_pre = {val_pre},break_val = {break_val},break_val_pre = {break_val_pre}")
        ...
        for i in range(24):
            ...
            ) + best_val * (output['f'][i] == 'SAF') * (output['j'][i] == 'NC') * output['Statetaxheaviside'][i] + (output['f'][i] == 'SAF') * best_model.getVarByName(SAFcredit_prices['P'].item().VarName).X * output.iloc[i]['SAFcreditheaviside']
        ...
        # if k == 0:
        #     # Save each DataFrame to an Excel file with a specified sheet name
        #     with pd.ExcelWriter(f'{output_dir1}Mean.xlsx', engine='openpyxl') as writer:
        #         Intermediate_var.to_excel(writer, index=False, sheet_name='Pure_quantity')

        # if k == 1:
        #     with pd.ExcelWriter(f'{output_dir1}10th.xlsx', engine='openpyxl') as writer:
        #         Intermediate_var.to_excel(writer, index=False, sheet_name='Pure_quantity')

        # if k == 2:
        #     with pd.ExcelWriter(f'{output_dir1}33th.xlsx', engine='openpyxl') as writer:
        #         Intermediate_var.to_excel(writer, index=False, sheet_name='Pure_quantity')

        # if k == 3:
        #     with pd.ExcelWriter(f'{output_dir1}67th.xlsx', engine='openpyxl') as writer:
        #         Intermediate_var.to_excel(writer, index=False, sheet_name='Pure_quantity')

        # if k == 4:
        #     with pd.ExcelWriter(f'{output_dir1}90th.xlsx', engine='openpyxl') as writer:
        #         Intermediate_var.to_excel(writer, index=False, sheet_name='Pure_quantity')                        
        ...
        if k == 0:
            append_to_excel(os.path.join(output_dir1, 'Mean.xlsx'), Intermediate_var, 'Pure_quantity')

        if k == 1:
            append_to_excel(os.path.join(output_dir1, '10th.xlsx'), Intermediate_var, 'Pure_quantity')

        if k == 2:
            append_to_excel(os.path.join(output_dir1, '33th.xlsx'), Intermediate_var, 'Pure_quantity')

        if k == 3:
            append_to_excel(os.path.join(output_dir1, '67th.xlsx'), Intermediate_var, 'Pure_quantity') 

        if k == 4:
            append_to_excel(os.path.join(output_dir1, '90th.xlsx'), Intermediate_var, 'Pure_quantity')
...
# with pd.ExcelWriter(f'{output_dir}Solution.xlsx', engine='openpyxl') as writer:
#     solution.to_excel(writer, index=False, sheet_name='Pure_quantity')

# with pd.ExcelWriter(f'{output_dir}Fitted_quantity.xlsx', engine='openpyxl') as writer:
#     Fitted_quantity.to_excel(writer, index=False, sheet_name='Pure_quantity')

# with pd.ExcelWriter(f'{output_dir}Fuel_price.xlsx', engine='openpyxl') as writer:
#     Fuel_price.to_excel(writer, index=False, sheet_name='Pure_quantity')

# with pd.ExcelWriter(f'{output_dir}Fuel_CI.xlsx', engine='openpyxl') as writer:
#     Fuel_CI.to_excel(writer, index=False, sheet_name='Pure_quantity')

# with pd.ExcelWriter(f'{output_dir}Total.xlsx', engine='openpyxl') as writer:
#     Total.to_excel(writer, index=False, sheet_name='Pure_quantity')

# with pd.ExcelWriter(f'{output_dir}Feedstock_source.xlsx', engine='openpyxl') as writer:
#     Feedstock_source.to_excel(writer, index=False, sheet_name='Pure_quantity')

# output_dir already defined above, no need to redefine
...
append_or_create_excel(solution, f'{output_dir}Solution.xlsx', 'Pure_quantity')
append_or_create_excel(Fitted_quantity, f'{output_dir}Fitted_quantity.xlsx', 'Pure_quantity') 
append_or_create_excel(Fuel_price, f'{output_dir}Fuel_price.xlsx', 'Pure_quantity')
append_or_create_excel(Fuel_CI, f'{output_dir}Fuel_CI.xlsx', 'Pure_quantity')
append_or_create_excel(Total, f'{output_dir}Total.xlsx', 'Pure_quantity')
append_or_create_excel(Feedstock_source, f'{output_dir}Feedstock_source.xlsx', 'Pure_quantity')
