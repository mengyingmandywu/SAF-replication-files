...
parser = argparse.ArgumentParser(description='Run biofuel policy optimization model (nonnested_D2_RV1) with configurable input/output folders.')
...
solution = pd.DataFrame(
    np.zeros((79, 6)),  # 70 rows and 6 columns filled with zeros
...
solution['Name'] = [
    'Biofuel quantity', 'Q_B100-soyoil-CA', 'Q_B100-animalfat-CA', 'Q_RD-soyoil-CA', 'Q_RD-animalfat-CA', 'Q_SAF-animalfat-CA', 'Q_gasE100-corn-CA',
                 ...
                 'Blended-Fuel', 'D_CA', 'G_CA', 'J_CA', 'D_NC', 'G_NC', 'J_NC',
                 'DGE_price',
...
Fitted_quantity['Name'] = [
    'RVO_D2', 'RVO_D4', 'RVO_RF', 'RV_D4', 'RV_RF', 'Fitted_BBD', 'Fitted_RF', 'Fitted_LCFS_deficit-credits','Fitted_SAF_LCFS_deficit-credits',
...
for k in range(K_RANGE):    
    # Read input_biofuel from generated V1 (OLD rules, NC-zeroed CI columns)
    intermediate_dir = os.path.join(parent_dir, 'intermediate')
    d2_path = os.path.join(intermediate_dir, 'input_biofuel_D2.xlsx')
    if not os.path.exists(d2_path):
        ...
        if _generate_outputs_main is not None:
            try:
                ...
                raise RuntimeError(f"Failed to generate input_biofuel_D2.xlsx via generate_outputs.py: {e}")
        else:
            raise FileNotFoundError(
                "input_biofuel_D2.xlsx not found and generate_outputs.py unavailable to build it."
            )
    if not os.path.exists(d2_path):
        raise FileNotFoundError(f"Expected generated file is missing: {d2_path}")

    input_biofuel = pd.read_excel(d2_path)
    ...
    input_compliance = pd.read_excel(compliance_path, sheet_name='V1', index_col=0)
    ...
    for break_idx, break_val in enumerate(breaks):
        ...
        # RIN prices
        RIN_prices_P = m.addVars(3, vtype=GRB.CONTINUOUS, name="RIN_prices_P")
        RIN_prices = pd.DataFrame({
            'P': [RIN_prices_P[i] for i in range(3)],
            'fueltype': ['SAF','B100,RD', 'gasE100'],
            'code': ['D2','D4', 'D6']
        }, index=['D2','D4', 'D6'])

        # RVO percent
        RVO_percent_P = m.addVars(3, vtype=GRB.CONTINUOUS, name="RVO_percent_P")
        RVO_percent = pd.DataFrame({
            'P': [RVO_percent_P[i] for i in range(3)],
            'mandate': ['D2','BBD', 'RF']
        }, index=['D2','BBD', 'RF'])
        ...
        conventionalfuel_quantity.loc['B0_CA':'E0_CA', 'CI_LCFS'] = input_compliance.loc[input_compliance['mandate'] == 'LCFS', 'CI_LCFS'].values
        ...
        conventionalfuel_quantity.loc['J0_CA', 'CI_std_LCFS'] = input_biofuel.loc[(input_biofuel['F'] == 'J') & (input_biofuel['j'] == 'CA'), 'CI_std_LCFS'].unique()[0]
        conventionalfuel_quantity.loc['B0_CA':'E0_CA', 'ED_LCFS'] = input_compliance.loc[input_compliance['mandate'] == 'LCFS', 'ED_LCFS'].values
        ...
        Price_conventional = pd.DataFrame({
            ...
            'P_RIN_LCFS_detach': [m.addVar(vtype=GRB.CONTINUOUS, name=f"P_RIN_LCFS_detach_{i+len(input_biofuel)}") for i in range(6)],
            'P_RIN_detach': [m.addVar(vtype=GRB.CONTINUOUS, name=f"P_RIN_detach_{i+len(input_biofuel)}") for i in range(6)],
        ...
        for i in range(24):
            Constraints.append(
                ...
                output['kappa_RFS'][i] * RIN_prices.loc[RIN_prices['code'] == 'D2', 'P'].item() * output['D2'][i] +
        ...
        # RVO percent
        # Constraints.append(
        #     RVO_percent.loc['D2', 'P'] == 
        #     3*1.6 / 
        #     (conventionalfuel_quantity.loc[(conventionalfuel_quantity['F'] == 'J'), 'Q'].sum()+denominator)
        # )

        Constraints.append(
            RVO_percent.loc['D2', 'P'] == 
            3*1.6 / 
            (conventionalfuel_quantity.loc[(conventionalfuel_quantity['F'] == 'J'), 'Q'].sum()+conventionalfuel_quantity.loc[(conventionalfuel_quantity['F'] == 'G') | (conventionalfuel_quantity['F'] == 'D'), 'Q'].sum())
        )

        # Constraints.append(RVO_percent.loc['BBD', 'P'] == 0)
        
        # Constraints.append(
        #         RVO_percent.loc['BBD', 'P'] == 
        #         (input_compliance.loc[input_compliance['mandate'] == 'BBD', 'Q'].item() * 
        #         conventionalfuel_quantity.loc[(conventionalfuel_quantity['F'] == 'G') | (conventionalfuel_quantity['F'] == 'D'), 'Q'].sum()/1.6-3)*1.6 / 
        #         (conventionalfuel_quantity.loc[(conventionalfuel_quantity['F'] == 'J'), 'Q'].sum()+conventionalfuel_quantity.loc[(conventionalfuel_quantity['F'] == 'G') | (conventionalfuel_quantity['F'] == 'D'), 'Q'].sum())
        #     )

        # # Define big-M and binary variable
        # bigM = 10
        # expr = input_compliance.loc[input_compliance['mandate'] == 'BBD', 'Q'].item() * \
        #     conventionalfuel_quantity.loc[(conventionalfuel_quantity['F'] == 'G') | (conventionalfuel_quantity['F'] == 'D'), 'Q'].sum() / 1.6
        
        # is_low = m.addVar(vtype=GRB.BINARY, name="is_low")
        
        # # Constraint to set is_low based on condition
        # Constraints.append(expr - 3 <= bigM * (1 - is_low))
        # Constraints.append(expr - 3 >= 0.0001 - bigM * is_low)
        
        # # Calculate original value
        # original_value = (expr - 3) * 1.6 / (
        ...
        # Constraints.append(RVO_percent.loc['BBD', 'P'] == is_low * 0 + (1 - is_low) * original_value)
        ...
        #     input_compliance.loc[input_compliance['mandate'] == 'RF', 'Q'].item()*conventionalfuel_quantity.loc[(conventionalfuel_quantity['F'] == 'G') | (conventionalfuel_quantity['F'] == 'D'), 'Q'].sum()/(conventionalfuel_quantity.loc[(conventionalfuel_quantity['F'] == 'J'), 'Q'].sum()+conventionalfuel_quantity.loc[(conventionalfuel_quantity['F'] == 'G') | (conventionalfuel_quantity['F'] == 'D'), 'Q'].sum())
        # )

        Constraints.append(
            RVO_percent.loc['RF', 'P'] == 
            (sum((biofuel_quantity.loc[biofuel_quantity['f'] == 'B100', 'kappa_RFS'] * biofuel_quantity.loc[biofuel_quantity['f'] == 'B100', 'Q'])) + sum(biofuel_quantity.loc[biofuel_quantity['f'] == 'RD', 'kappa_RFS'] * biofuel_quantity.loc[biofuel_quantity['f'] == 'RD', 'Q']) + sum(biofuel_quantity.loc[biofuel_quantity['f'] == 'SAF', 'kappa_RFS'] * biofuel_quantity.loc[biofuel_quantity['f'] == 'SAF', 'Q']) + sum(biofuel_quantity.loc[biofuel_quantity['f'] == 'gasE100', 'kappa_RFS'] * biofuel_quantity.loc[biofuel_quantity['f'] == 'gasE100', 'Q']))/(conventionalfuel_quantity.loc[(conventionalfuel_quantity['F'] == 'J'), 'Q'].sum()+conventionalfuel_quantity.loc[(conventionalfuel_quantity['F'] == 'G') | (conventionalfuel_quantity['F'] == 'D'), 'Q'].sum())
        )


       # Conventional fuel prices
        Fconventionalfuel_pricesD_NC = (conventionalfuel_prices.loc[(conventionalfuel_prices['F'] == 'D') & (conventionalfuel_prices['j'] == 'NC'), 'P'].item() -   RVO_percent.loc['D2', 'P'] * RIN_prices.loc['D2', 'P'] - 
            RVO_percent.loc['BBD', 'P'] * RIN_prices.loc['D4', 'P'] - 
            (RVO_percent.loc['RF', 'P'] - RVO_percent.loc['BBD', 'P'] - RVO_percent.loc['D2', 'P']) * RIN_prices.loc['D6', 'P'] - 
                                        input_demand.loc[input_demand['F'] == 'D', 'PC_F0'].unique()[0])
        Fconventionalfuel_pricesG_NC = (conventionalfuel_prices.loc[(conventionalfuel_prices['F'] == 'G') & (conventionalfuel_prices['j'] == 'NC'), 'P'].item() -  RVO_percent.loc['D2', 'P'] * RIN_prices.loc['D2', 'P'] - 
            RVO_percent.loc['BBD', 'P'] * RIN_prices.loc['D4', 'P'] - 
            (RVO_percent.loc['RF', 'P'] - RVO_percent.loc['BBD', 'P'] - RVO_percent.loc['D2', 'P']) * RIN_prices.loc['D6', 'P'] - 
                                        input_demand.loc[input_demand['F'] == 'G', 'PC_F0'].unique()[0])
        Fconventionalfuel_pricesJ_NC = (conventionalfuel_prices.loc[(conventionalfuel_prices['F'] == 'J') & (conventionalfuel_prices['j'] == 'NC'), 'P'].item() - RVO_percent.loc['D2', 'P'] * RIN_prices.loc['D2', 'P'] - 
            RVO_percent.loc['BBD', 'P'] * RIN_prices.loc['D4', 'P'] - 
            (RVO_percent.loc['RF', 'P'] - RVO_percent.loc['BBD', 'P'] - RVO_percent.loc['D2', 'P']) * RIN_prices.loc['D6', 'P'] -
                                        input_demand.loc[input_demand['F'] == 'J', 'PC_F0'].unique()[0])

        Fconventionalfuel_pricesD_CA = (conventionalfuel_prices.loc[(conventionalfuel_prices['F'] == 'D') & (conventionalfuel_prices['j'] == 'CA'), 'P'].item() - RVO_percent.loc['D2', 'P'] * RIN_prices.loc['D2', 'P'] - 
            RVO_percent.loc['BBD', 'P'] * RIN_prices.loc['D4', 'P'] - 
            (RVO_percent.loc['RF', 'P'] - RVO_percent.loc['BBD', 'P'] - RVO_percent.loc['D2', 'P']) * RIN_prices.loc['D6', 'P'] - 
                                        LCFS_prices.loc['LCFS', 'P'] * 
                                        (-conventionalfuel_quantity.loc[(conventionalfuel_quantity['j'] == 'CA') & (conventionalfuel_quantity['F'] == 'D'), 'CI_std_LCFS'].item() + 
                                        ...
                                        input_demand.loc[input_demand['F'] == 'D', 'PC_F0'].unique()[0])
        Fconventionalfuel_pricesG_CA = (conventionalfuel_prices.loc[(conventionalfuel_prices['F'] == 'G') & (conventionalfuel_prices['j'] == 'CA'), 'P'].item() - RVO_percent.loc['D2', 'P'] * RIN_prices.loc['D2', 'P'] - 
            RVO_percent.loc['BBD', 'P'] * RIN_prices.loc['D4', 'P'] - 
            (RVO_percent.loc['RF', 'P'] - RVO_percent.loc['BBD', 'P'] - RVO_percent.loc['D2', 'P']) * RIN_prices.loc['D6', 'P'] - 
                                        LCFS_prices.loc['LCFS', 'P'] * 
                                        (-conventionalfuel_quantity.loc[(conventionalfuel_quantity['j'] == 'CA') & (conventionalfuel_quantity['F'] == 'G'), 'CI_std_LCFS'].item() + 
                                        ...
                                        input_demand.loc[input_demand['F'] == 'G', 'PC_F0'].unique()[0])
        Fconventionalfuel_pricesJ_CA = (conventionalfuel_prices.loc[(conventionalfuel_prices['F'] == 'J') & (conventionalfuel_prices['j'] == 'CA'), 'P'].item() -  RVO_percent.loc['D2', 'P'] * RIN_prices.loc['D2', 'P'] - 
            RVO_percent.loc['BBD', 'P'] * RIN_prices.loc['D4', 'P'] - 
            (RVO_percent.loc['RF', 'P'] - RVO_percent.loc['BBD', 'P'] - RVO_percent.loc['D2', 'P']) * RIN_prices.loc['D6', 'P'] -
                                        input_demand.loc[input_demand['F'] == 'J', 'PC_F0'].unique()[0])
        ...
        Constraints.append(sum(Price.loc[(Price['F'] == 'D') & (Price['j'] == 'CA'), 'P_RIN_LCFS_detach'] * 
        ...
        Constraints.append(sum(Price.loc[(Price['F'] == 'D') & (Price['j'] == 'NC'), 'P_RIN_LCFS_detach'] * 
        ...
        Constraints.append(sum(Price.loc[(Price['F'] == 'G') & (Price['j'] == 'CA'), 'P_RIN_LCFS_detach'] * 
        ...
        Constraints.append(sum(Price.loc[(Price['F'] == 'G') & (Price['j'] == 'NC'), 'P_RIN_LCFS_detach'] * 
        ...
        Constraints.append(sum(Price.loc[(Price['F'] == 'J') & (Price['j'] == 'CA'), 'P_RIN_LCFS_detach'] * 
        ...
        Constraints.append(sum(Price.loc[(Price['F'] == 'J') & (Price['j'] == 'NC'), 'P_RIN_LCFS_detach'] * 
        ...
        for i in range(24):
            Constraints.append(
                Price.iloc[i]['RIN_obligation'] - (output['kappa_RFS'][i] * RIN_prices.loc[RIN_prices['code'] == 'D2', 'P'].item() * output['D2'][i] +
                output['kappa_RFS'][i] * RIN_prices.loc[RIN_prices['code'] == 'D4', 'P'].item() * output['D4'][i] +
        ...
        Constraints.append(
            ...
            RVO_percent.loc['D2', 'P'] * RIN_prices.loc['D2', 'P'] + 
            RVO_percent.loc['BBD', 'P'] * RIN_prices.loc['D4', 'P'] +
            (RVO_percent.loc['RF', 'P'] - RVO_percent.loc['BBD', 'P'] - RVO_percent.loc['D2', 'P']) * RIN_prices.loc['D6', 'P']
        )
        ...
        Constraints.append(
            Price.loc[(Price['f'] == 'J0') & (Price['j'] == 'CA'), 'RIN_obligation'].item() ==
            RVO_percent.loc['D2', 'P'] * RIN_prices.loc['D2', 'P'] + 
            RVO_percent.loc['BBD', 'P'] * RIN_prices.loc['D4', 'P'] +
            (RVO_percent.loc['RF', 'P'] - RVO_percent.loc['BBD', 'P'] - RVO_percent.loc['D2', 'P']) * RIN_prices.loc['D6', 'P']
        )
        ...
        Constraints.append(
            Price.loc[(Price['f'] == 'J0') & (Price['j'] == 'NC'), 'RIN_obligation'].item() ==
            RVO_percent.loc['D2', 'P'] * RIN_prices.loc['D2', 'P'] + 
            RVO_percent.loc['BBD', 'P'] * RIN_prices.loc['D4', 'P'] +
            (RVO_percent.loc['RF', 'P'] - RVO_percent.loc['BBD', 'P'] - RVO_percent.loc['D2', 'P']) * RIN_prices.loc['D6', 'P']
        )
        ...
        for i in range(24):
            Constraints.append(
                Price.iloc[i]['LCFS_CA_obligation'] - (LCFS_prices.loc['LCFS', 'P'] * (output.iloc[i]['CI_std_LCFS'] - output.iloc[i]['CI_LCFS']) *
        ...
        for i in range(24):
            Constraints.append(
                Price.iloc[i]['P_all'] - (
                    ...
                    (LCFS_prices.loc['LCFS', 'P'] * (output.iloc[i]['CI_std_LCFS'] - output.iloc[i]['CI_LCFS']) * output.iloc[i]['ED_LCFS'] * (output.iloc[i]['j'] == 'CA') / 1000000) 
                ) == 0
            )
        ...
        for i in range(24):
            Constraints.append(
                Price.iloc[i]['P_RIN_LCFS_detach'] - biofuel_prices.loc[(biofuel_prices['f'] == output.iloc[i]['f']) & (biofuel_prices['j'] == output.iloc[i]['j']), 'P'].item() == 0
            )

        Constraints.append(
            Price.loc[(Price['f'] == 'B0') & (Price['j'] == 'NC'), 'P_RIN_LCFS_detach'].item() -
            input_demand.loc[input_demand['F'] == 'D', 'PC_F0'].unique()[0] -  RVO_percent.loc['D2', 'P'] * RIN_prices.loc['D2', 'P'] - 
            RVO_percent.loc['BBD', 'P'] * RIN_prices.loc['D4', 'P'] - 
            (RVO_percent.loc['RF', 'P'] - RVO_percent.loc['BBD', 'P'] - RVO_percent.loc['D2', 'P']) * RIN_prices.loc['D6', 'P'] == 0
        )
        Constraints.append(
            Price.loc[(Price['f'] == 'E0') & (Price['j'] == 'NC'), 'P_RIN_LCFS_detach'].item() -
            input_demand.loc[input_demand['F'] == 'G', 'PC_F0'].unique()[0] -  RVO_percent.loc['D2', 'P'] * RIN_prices.loc['D2', 'P'] - 
            RVO_percent.loc['BBD', 'P'] * RIN_prices.loc['D4', 'P'] - 
            (RVO_percent.loc['RF', 'P'] - RVO_percent.loc['BBD', 'P'] - RVO_percent.loc['D2', 'P']) * RIN_prices.loc['D6', 'P'] == 0
        )
        Constraints.append(
            Price.loc[(Price['f'] == 'J0') & (Price['j'] == 'NC'), 'P_RIN_LCFS_detach'].item() -  RVO_percent.loc['D2', 'P'] * RIN_prices.loc['D2', 'P'] - 
            RVO_percent.loc['BBD', 'P'] * RIN_prices.loc['D4', 'P'] - 
            (RVO_percent.loc['RF', 'P'] - RVO_percent.loc['BBD', 'P'] - RVO_percent.loc['D2', 'P']) * RIN_prices.loc['D6', 'P'] -
        ...
        Constraints.append(
            Price.loc[(Price['f'] == 'B0') & (Price['j'] == 'CA'), 'P_RIN_LCFS_detach'].item() -
            input_demand.loc[input_demand['F'] == 'D', 'PC_F0'].unique()[0] -  RVO_percent.loc['D2', 'P'] * RIN_prices.loc['D2', 'P'] - 
            RVO_percent.loc['BBD', 'P'] * RIN_prices.loc['D4', 'P'] - 
            (RVO_percent.loc['RF', 'P'] - RVO_percent.loc['BBD', 'P'] - RVO_percent.loc['D2', 'P']) * RIN_prices.loc['D6', 'P'] -
            LCFS_prices.loc['LCFS', 'P'] * (
        ...
        Constraints.append(
            Price.loc[(Price['f'] == 'E0') & (Price['j'] == 'CA'), 'P_RIN_LCFS_detach'].item() - 
            input_demand.loc[input_demand['F'] == 'G', 'PC_F0'].unique()[0] -  RVO_percent.loc['D2', 'P'] * RIN_prices.loc['D2', 'P'] - 
            RVO_percent.loc['BBD', 'P'] * RIN_prices.loc['D4', 'P'] - 
            (RVO_percent.loc['RF', 'P'] - RVO_percent.loc['BBD', 'P'] - RVO_percent.loc['D2', 'P']) * RIN_prices.loc['D6', 'P'] -
            LCFS_prices.loc['LCFS', 'P'] * (
        ...
        Constraints.append(
            Price.loc[(Price['f'] == 'J0') & (Price['j'] == 'CA'), 'P_RIN_LCFS_detach'].item() -  RVO_percent.loc['D2', 'P'] * RIN_prices.loc['D2', 'P'] - 
            RVO_percent.loc['BBD', 'P'] * RIN_prices.loc['D4', 'P'] - 
            (RVO_percent.loc['RF', 'P'] - RVO_percent.loc['BBD', 'P'] - RVO_percent.loc['D2', 'P']) * RIN_prices.loc['D6', 'P'] -
            input_demand.loc[input_demand['F'] == 'J', 'PC_F0'].unique()[0] == 0
        )
        ...
        Constraints.append(
            Price.loc[(Price['f'] == 'B0') & (Price['j'] == 'CA'), 'P_RIN_detach'].item() -
            input_demand.loc[input_demand['F'] == 'D', 'PC_F0'].unique()[0] -  RVO_percent.loc['D2', 'P'] * RIN_prices.loc['D2', 'P'] - 
            RVO_percent.loc['BBD', 'P'] * RIN_prices.loc['D4', 'P'] - 
            (RVO_percent.loc['RF', 'P'] - RVO_percent.loc['BBD', 'P'] - RVO_percent.loc['D2', 'P']) * RIN_prices.loc['D6', 'P'] == 0
        )
        Constraints.append(
            Price.loc[(Price['f'] == 'E0') & (Price['j'] == 'CA'), 'P_RIN_detach'].item() -
            input_demand.loc[input_demand['F'] == 'G', 'PC_F0'].unique()[0] -  RVO_percent.loc['D2', 'P'] * RIN_prices.loc['D2', 'P'] - 
            RVO_percent.loc['BBD', 'P'] * RIN_prices.loc['D4', 'P'] - 
            (RVO_percent.loc['RF', 'P'] - RVO_percent.loc['BBD', 'P'] - RVO_percent.loc['D2', 'P']) * RIN_prices.loc['D6', 'P'] == 0
        )
        Constraints.append(
            Price.loc[(Price['f'] == 'J0') & (Price['j'] == 'CA'), 'P_RIN_detach'].item() -  RVO_percent.loc['D2', 'P'] * RIN_prices.loc['D2', 'P'] - 
            RVO_percent.loc['BBD', 'P'] * RIN_prices.loc['D4', 'P'] - 
            (RVO_percent.loc['RF', 'P'] - RVO_percent.loc['BBD', 'P'] - RVO_percent.loc['D2', 'P']) * RIN_prices.loc['D6', 'P'] -
            input_demand.loc[input_demand['F'] == 'J', 'PC_F0'].unique()[0] == 0
        )
        ...
        Constraints.append(
            Price.loc[(Price['f'] == 'B0') & (Price['j'] == 'NC'), 'P_RIN_detach'].item() -
            input_demand.loc[input_demand['F'] == 'D', 'PC_F0'].unique()[0] -  RVO_percent.loc['D2', 'P'] * RIN_prices.loc['D2', 'P'] - 
            RVO_percent.loc['BBD', 'P'] * RIN_prices.loc['D4', 'P'] - 
            (RVO_percent.loc['RF', 'P'] - RVO_percent.loc['BBD', 'P'] - RVO_percent.loc['D2', 'P']) * RIN_prices.loc['D6', 'P'] == 0
        )
        Constraints.append(
            Price.loc[(Price['f'] == 'E0') & (Price['j'] == 'NC'), 'P_RIN_detach'].item() -
            input_demand.loc[input_demand['F'] == 'G', 'PC_F0'].unique()[0] -  RVO_percent.loc['D2', 'P'] * RIN_prices.loc['D2', 'P'] - 
            RVO_percent.loc['BBD', 'P'] * RIN_prices.loc['D4', 'P'] - 
            (RVO_percent.loc['RF', 'P'] - RVO_percent.loc['BBD', 'P'] - RVO_percent.loc['D2', 'P']) * RIN_prices.loc['D6', 'P'] == 0
        )
        Constraints.append(
            Price.loc[(Price['f'] == 'J0') & (Price['j'] == 'NC'), 'P_RIN_detach'].item() -
            input_demand.loc[input_demand['F'] == 'J', 'PC_F0'].unique()[0] -  RVO_percent.loc['D2', 'P'] * RIN_prices.loc['D2', 'P'] - 
            RVO_percent.loc['BBD', 'P'] * RIN_prices.loc['D4', 'P'] - 
            (RVO_percent.loc['RF', 'P'] - RVO_percent.loc['BBD', 'P'] - RVO_percent.loc['D2', 'P']) * RIN_prices.loc['D6', 'P'] == 0
        )
        ...
        PD2zeta = m.addVar(vtype=GRB.BINARY, name="PD2zeta")
        ...
        M11_0 = 5
        # # Add constraints for RIN prices and biofuel quantities
        Constraints.append((RIN_prices.loc['D2', 'P'] - RIN_prices.loc['D6', 'P']) >= 0)
        Constraints.append((RIN_prices.loc['D2', 'P'] - RIN_prices.loc['D6', 'P']) <= (1 - PD2zeta) * M11_0)

        # # Calculate the sum of SAF biofuel quantities
        # sum_SAF_biofuel_quantity = biofuel_quantity.loc[biofuel_quantity['f'] == 'SAF', 'Q'].sum()

        Constraints.append(biofuel_quantity.loc[biofuel_quantity['D2'] == 1, 'Q'].sum() - input_compliance.loc[input_compliance['mandate'] == 'SAF', 'Q'].item() >= 0)
        Constraints.append(biofuel_quantity.loc[biofuel_quantity['D2'] == 1, 'Q'].sum() - input_compliance.loc[input_compliance['mandate'] == 'SAF', 'Q'].item() <= PD2zeta * M11_0)

        M11_1 = 10
        Constraints.append((RIN_prices.loc['D4', 'P'] - RIN_prices.loc['D6', 'P']) >= 0)
        Constraints.append((RIN_prices.loc['D4', 'P'] - RIN_prices.loc['D6', 'P']) <= (1 - PD4zeta) * M11_1)


        # Constraints.append(0 <= (sum(biofuel_quantity.loc[biofuel_quantity['f'].isin(['B100', 'RD']), 'Q']) - 
        #                         RVO_percent.loc['BBD', 'P'] / 1.6 * 
        #                         (conventionalfuel_quantity.loc[(conventionalfuel_quantity['F'] == 'J'), 'Q'].sum()+conventionalfuel_quantity.loc[(conventionalfuel_quantity['F'] == 'G') | (conventionalfuel_quantity['F'] == 'D'), 'Q'].sum())))
        # Constraints.append((sum(biofuel_quantity.loc[biofuel_quantity['f'].isin(['B100', 'RD']), 'Q']) - 
        #                     RVO_percent.loc['BBD', 'P'] / 1.6 * 
        #                     (conventionalfuel_quantity.loc[(conventionalfuel_quantity['F'] == 'J'), 'Q'].sum()+conventionalfuel_quantity.loc[(conventionalfuel_quantity['F'] == 'G') | (conventionalfuel_quantity['F'] == 'D'), 'Q'].sum())) <= PD4zeta * M11_1)
        ...
        Constraints.append((biofuel_quantity.loc[(biofuel_quantity['D4'] == 1), 'Q'].sum() -
            (input_compliance.loc[input_compliance['mandate'] == 'BBD', 'Q'].item() / 1.6 * conventionalfuel_quantity.loc[conventionalfuel_quantity['F'].isin(['G', 'D']), 'Q'].sum()-3)) >= 0)
        Constraints.append((biofuel_quantity.loc[(biofuel_quantity['D4'] == 1), 'Q'].sum() -
            (input_compliance.loc[input_compliance['mandate'] == 'BBD', 'Q'].item() / 1.6 * conventionalfuel_quantity.loc[conventionalfuel_quantity['F'].isin(['G', 'D']), 'Q'].sum()-3)) <= PD4zeta * M11_1)
        ...
        # Constraints.append((sum(biofuel_quantity.loc[biofuel_quantity['f'] == 'B100', 'kappa_RFS'] * biofuel_quantity.loc[biofuel_quantity['f'] == 'B100', 'Q']) +
        #     sum(biofuel_quantity.loc[biofuel_quantity['f'] == 'RD', 'kappa_RFS'] * biofuel_quantity.loc[biofuel_quantity['f'] == 'RD', 'Q']) +
        #     sum(biofuel_quantity.loc[biofuel_quantity['f'] == 'SAF', 'kappa_RFS'] * biofuel_quantity.loc[biofuel_quantity['f'] == 'SAF', 'Q']) +
        #     sum(biofuel_quantity.loc[biofuel_quantity['f'] == 'gasE100', 'kappa_RFS'] * biofuel_quantity.loc[biofuel_quantity['f'] == 'gasE100', 'Q']) -
        #     RVO_percent.loc['RF', 'P'] * (sum(conventionalfuel_quantity.loc[conventionalfuel_quantity['F'].isin(['J']), 'Q'])+conventionalfuel_quantity.loc[(conventionalfuel_quantity['F'] == 'G') | (conventionalfuel_quantity['F'] == 'D'), 'Q'].sum())
        # )>=0)

        # Constraints.append((
        #     sum(biofuel_quantity.loc[biofuel_quantity['f'] == 'B100', 'kappa_RFS'] * biofuel_quantity.loc[biofuel_quantity['f'] == 'B100', 'Q']) +
        #     sum(biofuel_quantity.loc[biofuel_quantity['f'] == 'RD', 'kappa_RFS'] * biofuel_quantity.loc[biofuel_quantity['f'] == 'RD', 'Q']) +
        #     sum(biofuel_quantity.loc[biofuel_quantity['f'] == 'SAF', 'kappa_RFS'] * biofuel_quantity.loc[biofuel_quantity['f'] == 'SAF', 'Q']) +
        #     sum(biofuel_quantity.loc[biofuel_quantity['f'] == 'gasE100', 'kappa_RFS'] * biofuel_quantity.loc[biofuel_quantity['f'] == 'gasE100', 'Q']) -
        #     RVO_percent.loc['RF', 'P'] * (sum(conventionalfuel_quantity.loc[conventionalfuel_quantity['F'].isin(['J']), 'Q'])+conventionalfuel_quantity.loc[(conventionalfuel_quantity['F'] == 'G') | (conventionalfuel_quantity['F'] == 'D'), 'Q'].sum())
        ...
        Constraints.append((sum(biofuel_quantity.loc[biofuel_quantity['f'] == 'B100', 'kappa_RFS'] * biofuel_quantity.loc[biofuel_quantity['f'] == 'B100', 'Q']) +
            ...
            sum(biofuel_quantity.loc[biofuel_quantity['f'] == 'SAF', 'kappa_RFS'] * biofuel_quantity.loc[biofuel_quantity['f'] == 'SAF', 'Q']) +
            sum(biofuel_quantity.loc[biofuel_quantity['f'] == 'gasE100', 'kappa_RFS'] * biofuel_quantity.loc[biofuel_quantity['f'] == 'gasE100', 'Q']) -
            input_compliance.loc[input_compliance['mandate'] == 'RF', 'Q'].item()*conventionalfuel_quantity.loc[(conventionalfuel_quantity['F'] == 'G') | (conventionalfuel_quantity['F'] == 'D'), 'Q'].sum()
        )>=0)
        ...
        if m.status == GRB.OPTIMAL:
            ...
            for i in range(24):
                ...
                v3 = output['kappa_RFS'][i] * RIN_prices.loc[RIN_prices['code'] == 'D2', 'P'].item().X * output['D2'][i] + output['kappa_RFS'][i] * RIN_prices.loc[RIN_prices['code'] == 'D4', 'P'].item().X * output['D4'][i] + output['kappa_RFS'][i] * RIN_prices.loc[RIN_prices['code'] == 'D6', 'P'].item().X * output['D6'][i]
    ...
    if best_df is not None:
        ...
        for i in range(24):
            ...
            Intermediate_var.at[i, 'RFS_road'] = (
                (output['f'][i] in ['B100', 'RD', 'gasE100'])
                * (
                    output['kappa_RFS'][i] * best_model.getVarByName(RIN_prices.loc[RIN_prices['code'] == 'D2', 'P'].item().VarName).X * output['D2'][i] +
        ...
        for col in ['Tax', 'P_all', 'PC_F0', 'P_RIN_LCFS_detach', 'P_RIN_detach', 'Q_actual', 'Q_equiv']:
        ...
        if k == 0:
            append_to_excel(os.path.join(output_dir1, 'Mean.xlsx'), Intermediate_var, 'Nonnested_D2')

        if k == 1:
            append_to_excel(os.path.join(output_dir1, '10th.xlsx'), Intermediate_var, 'Nonnested_D2')

        if k == 2:
            append_to_excel(os.path.join(output_dir1, '33th.xlsx'), Intermediate_var, 'Nonnested_D2')

        if k == 3:
            append_to_excel(os.path.join(output_dir1, '67th.xlsx'), Intermediate_var, 'Nonnested_D2') 

        if k == 4:
            append_to_excel(os.path.join(output_dir1, '90th.xlsx'), Intermediate_var, 'Nonnested_D2')
        ...
        solution.iloc[:, k+1] = (
            ...
            #+ [0]
            + [best_model.getVarByName(RIN_prices_P[i].VarName).X for i in range(3)]
        ...
        Fitted_quantity.iloc[0:20, k+1] = [best_model.getVarByName(RVO_percent.loc[RVO_percent['mandate'] == 'D2', 'P'].values[0].VarName).X, best_model.getVarByName(RVO_percent.loc[RVO_percent['mandate'] == 'BBD', 'P'].values[0].VarName).X,
            ...
            best_model.getVarByName(RVO_percent.loc[RVO_percent['mandate'] == 'BBD', 'P'].values[0].VarName).X *
                get_x(conventionalfuel_quantity.loc[(conventionalfuel_quantity['F'].isin(['G', 'D', 'J'])), 'Q']) / 1.6,
            best_model.getVarByName(RVO_percent.loc[RVO_percent['mandate'] == 'RF', 'P'].values[0].VarName).X *
                get_x(conventionalfuel_quantity.loc[(conventionalfuel_quantity['F'].isin(['G', 'D', 'J'])), 'Q']),
            #get_x(biofuel_quantity.loc[biofuel_quantity['f'].isin(['B100', 'RD', 'SAF']), 'Q']),
            get_x(biofuel_quantity.loc[(biofuel_quantity['D4'] == 1) | (biofuel_quantity['D2'] == 1), 'Q']),
            sum(
            ...
            -(
                sum(
                    ...
                    for q, ci_std, ci, ed in zip(
                        ...
                        biofuel_quantity.loc[biofuel_quantity['j'] == 'CA', 'CI_std_LCFS'],
        ...
        Fuel_price.iloc[1:9, k+1] = [
            sum(Intermediate_var.loc[(Intermediate_var['j'] == 'CA') & (Intermediate_var['F'] == 'D'), 'P_RIN_LCFS_detach'
            ...
            sum(Intermediate_var.loc[(Intermediate_var['j'] == 'NC') & (Intermediate_var['F'] == 'D'), 'P_RIN_LCFS_detach'] *
            ...
            sum(Intermediate_var.loc[(Intermediate_var['j'] == 'CA') & (Intermediate_var['F'] == 'G'), 'P_RIN_LCFS_detach'
            ...
            sum(Intermediate_var.loc[(Intermediate_var['j'] == 'NC') & (Intermediate_var['F'] == 'G'), 'P_RIN_LCFS_detach'] *
            ...
            sum(Intermediate_var.loc[(Intermediate_var['j'] == 'CA') & (Intermediate_var['F'] == 'J'), 'P_RIN_LCFS_detach'
            ...
            sum(Intermediate_var.loc[(Intermediate_var['j'] == 'NC') & (Intermediate_var['F'] == 'J'), 'P_RIN_LCFS_detach'] *
            ...
            sum(Intermediate_var.loc[(Intermediate_var['j'] == 'CA') & (Intermediate_var['f'] == 'gasE100'), 'P_RIN_LCFS_detach'] *
            ...
            sum(Intermediate_var.loc[(Intermediate_var['j'] == 'NC') & (Intermediate_var['f'] == 'gasE100'), 'P_RIN_LCFS_detach'] *
        ...
        Fuel_price.iloc[9, k+1] = ((Fuel_price.iloc[7, k+1] * 0.1 + Intermediate_var.loc[(Intermediate_var['j'] == 'CA') & (Intermediate_var['f'] == 'E0'), 'P_RIN_LCFS_detach'].values[0] * 0.9) /(0.1 * input_biofuel.loc[input_biofuel['f'] == 'gasE100', 'phi_ed'].unique()[0] + 0.9))
        
        Fuel_price.iloc[10, k+1] = ((Fuel_price.iloc[8, k+1] * 0.1 + Intermediate_var.loc[(Intermediate_var['j'] == 'NC') & (Intermediate_var['f'] == 'E0'), 'P_RIN_LCFS_detach'].values[0] * 0.9) /(0.1 * input_biofuel.loc[input_biofuel['f'] == 'gasE100', 'phi_ed'].unique()[0] + 0.9))
        ...
        Fuel_price.iloc[24, k+1] = [
            ...
            best_model.getVarByName(RVO_percent_P[1].VarName).X * best_model.getVarByName(RIN_prices_P[1].VarName).X +
            (best_model.getVarByName(RVO_percent_P[2].VarName).X - best_model.getVarByName(RVO_percent_P[1].VarName).X - best_model.getVarByName(RVO_percent_P[0].VarName).X) * best_model.getVarByName(RIN_prices_P[2].VarName).X
        ]

        Fuel_price.iloc[25, k+1] = [
            best_model.getVarByName(RVO_percent_P[0].VarName).X * best_model.getVarByName(RIN_prices_P[0].VarName).X +
            best_model.getVarByName(RVO_percent_P[1].VarName).X * best_model.getVarByName(RIN_prices_P[1].VarName).X +
            (best_model.getVarByName(RVO_percent_P[2].VarName).X - best_model.getVarByName(RVO_percent_P[1].VarName).X - best_model.getVarByName(RVO_percent_P[0].VarName).X) * best_model.getVarByName(RIN_prices_P[2].VarName).X
        ]
        ...
        Total.iloc[36, k+1] = sum(
                Intermediate_var_subsidy.loc[Intermediate_var['F'] == 'D','LCFS'] * Intermediate_var_subsidy.loc[Intermediate_var['F'] == 'D','Q_actual']
            )

        Total.iloc[37, k+1] = sum(
                Intermediate_var_subsidy.loc[Intermediate_var['F'] == 'G','LCFS'] * Intermediate_var_subsidy.loc[Intermediate_var['F'] == 'G','Q_actual']
            )
        
        Total.iloc[38, k+1] = sum(
                Intermediate_var_subsidy.loc[Intermediate_var['F'] == 'J','LCFS'] * Intermediate_var_subsidy.loc[Intermediate_var['F'] == 'J','Q_actual']
            )
...
# Define the output directory using results_dir from command-line arguments
...
append_to_excel(f'{output_dir}Solution.xlsx', solution, 'Nonnested_D2')
append_to_excel(f'{output_dir}Fitted_quantity.xlsx', Fitted_quantity, 'Nonnested_D2')
append_to_excel(f'{output_dir}Fuel_price.xlsx', Fuel_price, 'Nonnested_D2')
append_to_excel(f'{output_dir}Fuel_CI.xlsx', Fuel_CI, 'Nonnested_D2')
append_to_excel(f'{output_dir}Total.xlsx', Total, 'Nonnested_D2')
append_to_excel(f'{output_dir}Feedstock_source.xlsx', Feedstock_source, 'Nonnested_D2')
